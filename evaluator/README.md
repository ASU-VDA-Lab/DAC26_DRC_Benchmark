...
Common fields (both tasks): `agent_status`, `runtime_seconds`, `evaluator_hash`. When `RECORD_TOKENS=1`, the four token fields (`input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_write_tokens`) and `num_calls` are also emitted; `RECORD_TOKENS=0` (default) omits those five fields.
